...
# interface for the AI Wi-Fi Warlord. It simulates the behavior of a real
# BLE library (like 'bleak' or 'pygatt') for development and testing purposes
# when no actual Bluetooth hardware is present.
...
class MockBLE:
    ...
    """
    def __init__(self, device_name="WarlordAI_BLE"):
    ...
    def start_gatt_server(self):
        ...
        if not self.is_listening:
            ...
            # This would typically run in a background event loop.
            # We can simulate receiving a command after a delay.
    ...
    def notify_status(self, status_data):
        ...
        In a real BLE implementation, this would write to a characteristic's value.
        """
        if self.is_listening and self.status_subscribers:
            ...
            # In a real scenario, you would iterate through connected clients
            # and send the data.
        else:
            # This would be too verbose to print on every status update,
            # so we'll keep it silent unless a client is "connected".
            pass

    def _simulate_command_reception(self):
        """A private method to simulate a command being received via BLE."""
        if self.is_listening:
            print("[BLE Mock] Simulated command received: 'toggle_ai_state'")
            # Here, you could link this back to the main application's state
            # to simulate remote control, for example, by calling a function
            # passed during initialization.
            # For now, it just prints a message.
            
            # Simulate a client "subscribing" to status updates.
